...
#着差の設定
label8 =tk.Label(root, text = '1,2')
box8 = tk.Entry(root)
label9 =tk.Label(root, text = '2,3')
box9 = tk.Entry(root)
label10 =tk.Label(root, text = '3,4')
box10 = tk.Entry(root)
label11 =tk.Label(root, text = '4,5')
box11 = tk.Entry(root)

#馬場状態の設定
label12 = tk.Label(root, text = '芝')
box12 = ttk.Combobox(root, state="readonly", values=['　','良','稍重','重','不良'], width = 4)
label13 = tk.Label(root, text = 'ダート')
box13 = ttk.Combobox(root, state="readonly", values=['　','良','稍重','重','不良'], width = 4)

#確定状況の設定
label14 = tk.Label(root, text = '状況')
box14 = ttk.Combobox(root, state="readonly", values=['　','確定','審議'], width = 4)

#タイム関連の設定
label15 = tk.Label(root, text = 'レコード')
box15 = ttk.Combobox(root, state="readonly", values=['　','ﾚｺｰﾄﾞ','ﾃｽﾄ*'], width = 4)
#入線タイム
label16 =tk.Label(root, text = 'time min.')
box16 = tk.Entry(root)
label17 =tk.Label(root, text = 'time sec')
box17 = tk.Entry(root)
label18 =tk.Label(root, text = 'time sec 1/10')
box18 = tk.Entry(root)
#4ハロン
label19 =tk.Label(root, text = '4F sec')
box19 = tk.Entry(root)
label20 =tk.Label(root, text = '4F sec 1/10')
box20 = tk.Entry(root)
#3ハロン
label21 =tk.Label(root, text = '3F sec')
box21 = tk.Entry(root)
label22 =tk.Label(root, text = '3F sec 1/10')
box22 = tk.Entry(root)
